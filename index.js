...
var client = mqtt.connect("mqtt://200.32.43.13", mqtt_options);

var temp = 0;

client.on('connect', function() { 
    console.log('connected');
    client.subscribe("bot/temperature");
});


client.on('message', function (topic, message) {
  if (topic == "bot/temperature") { 
	  ...
	  if (!translator){
		...
		if (received.includes("hola")) {
			res.send("Hola. ¿Como te puedo ayudar?");
		 	...
		 	res.send("En este momento hace " + temp + " grados centigrados");
